...
parser.add_argument('--sampling_method', type=str, default="kmeancosine")
...
parser.add_argument('--source_sampon', action="store_true")        # use sampler for source or not
...
# if space is fbnet, then metric device is titanxp_1
if args.space == "fbnet":
    args.metric_device = 'gold_6240'

# if space is nb201, then metric device is titanxp_1
if args.space == "nb201":
    args.metric_device = '1080ti_32'
...
def get_distinct_cosine_centers_avg(arch2vecs_, n):
    try:
        ...
        return random.sample(list(range(len(arch2vecs_))), n)
...
def get_distinct_index(args, embedding_gen, space, sample_count, metric, device): # [random, params, arc2vec, cate, zcp, a2vcatezcp, accuracy/latency (oracle)]
    if metric == 'random':
        ...
        elif args.sampling_method == 'kmeancosine':
            kmeansamps = get_distinct_arch2vecs_kmeans(metricdict_, sample_count//2)
            cosinesamps = get_distinct_cosine_centers_avg(metricdict_, sample_count - sample_count//2)
            return kmeansamps + cosinesamps
...
opfpgcn,fgcn,rbgcn,efm,fcd,lr,weight_decay,epochs,transfer_epochs,cpu_gpu_device,hwemb_to_mlp,transfer_hwemb,source_sampon,spr,kdt,spr_std,kdt_std"
